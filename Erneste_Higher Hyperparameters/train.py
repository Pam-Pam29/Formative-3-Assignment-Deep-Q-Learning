...
================================================
  ...
  Erneste — 10 Experiments + expBEST
  Higher Hyperparameter Sweep
...
# ── 10 Experiments — Higher Hyperparameter Sweep ──────────────────────────────
#
#  Strategy: Explore aggressive hyperparameter ranges
#  lr          : 0.0005 – 0.001   (higher learning rates)
#  gamma       : 0.990 – 0.999    (long-horizon discount)
#  batch_size  : 128 – 256        (larger batches)
#  eps_end     : 0.10 – 0.15      (more exploration retained)
#  eps_fraction: 0.15 – 0.30      (slower epsilon decay)
#
EXPERIMENTS = [
    dict(exp=1,  lr=0.0005, gamma=0.990, batch=128, eps_start=1.0, eps_end=0.10, eps_frac=0.15),
    dict(exp=2,  lr=0.0005, gamma=0.995, batch=128, eps_start=1.0, eps_end=0.10, eps_frac=0.20),
    dict(exp=3,  lr=0.0005, gamma=0.999, batch=256, eps_start=1.0, eps_end=0.10, eps_frac=0.20),
    dict(exp=4,  lr=0.0007, gamma=0.990, batch=128, eps_start=1.0, eps_end=0.10, eps_frac=0.15),
    dict(exp=5,  lr=0.0007, gamma=0.995, batch=256, eps_start=1.0, eps_end=0.10, eps_frac=0.20),
    dict(exp=6,  lr=0.0007, gamma=0.999, batch=256, eps_start=1.0, eps_end=0.15, eps_frac=0.25),
    dict(exp=7,  lr=0.0010, gamma=0.990, batch=128, eps_start=1.0, eps_end=0.10, eps_frac=0.15),
    dict(exp=8,  lr=0.0010, gamma=0.995, batch=256, eps_start=1.0, eps_end=0.15, eps_frac=0.25),
    dict(exp=9,  lr=0.0010, gamma=0.999, batch=256, eps_start=1.0, eps_end=0.15, eps_frac=0.30),
    dict(exp=10, lr=0.0007, gamma=0.999, batch=128, eps_start=1.0, eps_end=0.10, eps_frac=0.25),
]

# Best config derived from sweep — lr=0.001 + gamma=0.995 + batch=256
BEST_CONFIG = dict(
    exp=11, lr=0.0010, gamma=0.995, batch=256,
    eps_start=1.0, eps_end=0.10, eps_frac=0.20,
)
...
def run_experiment(cfg, tag=None):
    exp_id       = cfg["exp"]
    ...
    print(f"\n  {tag} done — Mean Reward: {mean_reward:.2f}  Std: {std_reward:.2f}")
    print(f"  Note: {noted}")
    print(f"  Log  → {exp_csv}")
...
# ── Run 10 experiments ────────────────────────────────────────────────────────
print("=" * 60)
print("  Erneste — DQN Breakout Hyperparameter Sweep")
...
for row in summary_rows:
    print(f"  {row['exp']:>4} {row['lr']:>8} {row['gamma']:>6} {row['batch']:>6} "
          f"{row['eps_end']:>8} {row['eps_frac']:>9} "
          f"{row['mean_reward']:>9.2f} {row['std_reward']:>6.2f}")
...
print(f"  Best of 10  : exp{best10['exp']:02d} — {best10['mean_reward']:.2f}")

# ── Run expBEST ───────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("  Running expBEST — Optimised Config")
print("  lr=0.001  gamma=0.995  batch=256  eps_end=0.10  eps_frac=0.20")
print("=" * 60)

best_row = run_experiment(BEST_CONFIG, tag="expBEST")
